...
@section('title') Desafíos @parent @endsection
...
<script>
    ...
    $(document).on('click', '#deleteChallenges', function (e) {
        if ($('.contacts-checkbox.selectable-item:checked').length) {
            bootbox.dialog({
                message: "¿Estás seguro que quieres eliminar esto(s) desafío(s)?",
                buttons: {
                    cancel: {
                        label: 'Cancelar',
                    ...
                    success: {
                        ...
                        callback: function () {
                            $('.contacts-checkbox.selectable-item:checked').each(function () {
                                var id = $(this).data('challengeid');
                                $.post('/challenges/' + id, {'_token': '{{ csrf_token() }}', '_method': 'DELETE'}, function (data) {
                                    if (data == 'ok') {
                                        $('#challenge_' + id).closest('tr').remove();
                                        toastr.success('Desafío eliminado!', '', {});
                                    } else {
    ...
    $(document).on('click', '#deleteTrashedChallenges', function (e) {
        if ($('.contacts-checkbox.selectable-item:checked').length) {
            bootbox.dialog({
                message: "¿Estás seguro que quieres eliminar definitivamente esto(s) desafío(s)?",
                buttons: {
                    cancel: {
                        label: 'Cancelar',
                        className: 'btn-primary'
                    ...
                    success: {
                        ...
                        callback: function () {
                            $('.contacts-checkbox.selectable-item:checked').each(function () {
                                var id = $(this).data('challengeid');
                                $.post('/challenges/trashed/delete/' + id, {'_token': '{{ csrf_token() }}', '_method': 'POST'}, function (data) {
                                    if (data == 'ok') {
                                        $('#challenge_' + id).closest('tr').remove();
                                        toastr.success('Desafío eliminado!', '', {});
                                    } else {
    ...
    $(document).on('click', '#restoreTrashedChallenges', function (e) {
        if ($('.contacts-checkbox.selectable-item:checked').length) {
            bootbox.dialog({
                message: "¿Estás seguro que quieres restaurar esto(s) desafío(s)?",
                buttons: {
                    cancel: {
                        label: 'Cancelar',
                        className: 'btn-primary'
                    ...
                    success: {
                        ...
                        callback: function () {
                            $('.contacts-checkbox.selectable-item:checked').each(function () {
                                var id = $(this).data('challengeid');
                                $.post('/challenges/trashed/restore/' + id, {'_token': '{{ csrf_token() }}', '_method': 'POST'}, function (data) {
                                    if (data == 'ok') {
                                        $('#challenge_' + id).closest('tr').remove();
                                        toastr.success('Desafío restaurado!', '', {});
                                    } else {
                                        toastr.error(data, 'Error', {});
                                    }
                                });
...
@section('content')
    ...
    <div class="page bg-white animsition">
                    ...
                    <div data-role="content">
                        <div class="page-aside-section">
                            <div class="list-group">
                                <a class="list-group-item" href="/challenges">
                                    <span class="item-right">{{ \App\Manager\Challenges\Challenge::count() }}</span><i class="icon md-accounts-list"
                                                                                               aria-hidden="true"></i> Todos los desafíos
                                </a>
                                <a class="list-group-item" href="/challenges/trashed">
                                    <span class="item-right">{{ \App\Manager\Challenges\Challenge::onlyTrashed()->count() }}</span><i class="icon md-delete"
                                                                                                              aria-hidden="true"></i> Desafíos eliminados
                                </a>
                            </div>
                        </div>
                        <div class="page-aside-section">
                            ...
                            <div class="list-group">
                                @foreach($programs as $pr)
                                    <a class="list-group-item" href="/challenges/program/{{ $pr->id }}">
                                        <span class="item-right">{{ $pr->challenges->count() }}</span>{{ $pr->name }}
                                    </a>
                                @endforeach
                            </div>
                        </div>
        ...
        <div class="page-main">
            ...
            <div class="page-header">
                <h1 class="page-title">
                    Lista de desafíos <small>{{ ($option=='trashed')?' - Eliminados ':($option=='program'?'Programa: ' . $program->name:'Todos') }}</small>
                </h1>
            ...
            <div class="page-content page-content-table">
                ...
                <div class="page-content-actions">
                    <div class="pull-right">
                        @if($option=="trashed")
                            <form action="/challenges/deltrashed" method="post">
                                {{ csrf_field() }}
                                {{ method_field('POST') }}
                                <button id="deleteTrashedChallenges" type="button" class="btn btn-danger btn-md empty-btn"><i
                                            class="icon md-delete"></i>Eliminar definitivamente
                                </button>
                                <button id="restoreTrashedChallenges" type="button" class="btn btn-primary btn-md empty-btn"><i class="icon md-edit"></i>Restaurar
                                </button>
                            </form>
                        @else
                            <form action="/challenges" method="post">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                <button id="deleteChallenges" type="button" class="btn btn-danger btn-md empty-btn"><i class="icon md-delete"></i>Eliminar
                                </button>
                            </form>
                        @endif
                    </div>
                ...
                <span class="checkbox-custom checkbox-primary checkbox-lg contacts-select-all">
                  ...
                  <label for="select_all"></label>
                        ...
                        <th class="cell-300" scope="col" data-tablesaw-sortable-col data-tablesaw-priority="3">Nombre / Programa</th>
                        <th class="cell-300" scope="col" data-tablesaw-sortable-col data-tablesaw-priority="3">Metas</th>
                    ...
                    @foreach($challenges as $challenge)
                        <tr>
                            ...
                            <span class="checkbox-custom checkbox-primary checkbox-lg">
                              <input type="checkbox" class="contacts-checkbox selectable-item" id="challenge_{{ $challenge->id }}" data-challengeid="{{ $challenge->id }}"/>
                              <label for="challenge_{{ $challenge->id }}"></label>
                            </span>
                            </td>
                            <td class="cell-300">
                                {{ $challenge->name }}<br>
                                <small>{{ $challenge->program?$challenge->program->name:'-' }}<br>
                                <span class="text-info">{{ implode(', ',$challenge->channels->pluck('name')->toArray()) }}</span>
                                </small>
                            </td>
                            <td class="cell-300">
                                @forelse($challenge->goals as $gl)
                                    <span class="text-info">{{ $gl->name }}</span><br>
                                @empty
                                    No hay metas asociadas a este desafío
                                @endforelse
                            ...
                            @if($option != 'trashed')
                                <td>
                                    <button class="btn btn-primary" data-url="/challenges/{{  $challenge->id }}/edit" data-toggle="slidePanel">Editar</button>
                                </td>
                            @endif
    ...
    <!-- Site Action -->
    <button class="site-action btn-raised btn btn-success btn-floating" data-target="#addChallengeForm"
            data-toggle="modal" type="button">
        <i class="icon md-plus" aria-hidden="true"></i>
    </button>
    <!-- End Site Action -->
    <!-- Add User Form -->
    <div class="modal fade" id="addChallengeForm" aria-hidden="true" aria-labelledby="addChallengeForm"
         role="dialog" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" aria-hidden="true" data-dismiss="modal">×</button>
                    <h4 class="modal-title">Agregar Desafío</h4>
                </div>
